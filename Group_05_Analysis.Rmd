...
library(gridExtra)
library(epiDisplay)
setwd("C:/Users/Lenovo/Desktop/GLM")
...
The histgram of response variable total member in each family shows that it is counting variable and seems to follow possion distribution. So Possion regression model is proper for this study.
...
By excluding the non-significant independent variables under the new model, we get the final model5.
...
```{r}
c(model0$deviance, model0$aic)
c(model00$deviance, model00$aic)
...
```{r}
pred<-predict(model, household)
plot(household$Total.Number.of.Family.members,exp(pred))
abline(a=0,b=1)
```
